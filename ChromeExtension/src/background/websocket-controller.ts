...
function getChromeInternalPageIcon(hostname: string): string {
...
// タブ情報を登録(リトライ処理付き)
export function webSocketRequestUpdateTab(webSocketClient: WebSocketClient, tab: chrome.tabs.Tab, status: string | null = null, retryCount: number = 0): void {
    const maxRetries = 3;
    ...
    if (!webSocketClient.getConnectionStatus()) {
        if (retryCount < maxRetries) {
            ...
            setTimeout(() => {
                webSocketRequestUpdateTab(webSocketClient, tab, status, retryCount + 1);
            }, retryDelay);
        } else {
...
// タブ削除を通知
export function webSocketRequestRemoveTab(webSocketClient: WebSocketClient, tabId: number, windowId: number): void {
    if (!webSocketClient.getConnectionStatus()) {
